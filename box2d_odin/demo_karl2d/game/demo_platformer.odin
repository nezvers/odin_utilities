...
process :: proc() {
    b2_odin.WorldUpdate(&world_ctx, karl2d.get_frame_time())
    
    // player input update
    player.input.jump = karl2d.key_is_held(.Space)
    right:bool = karl2d.key_is_held(.D)
    left:bool = karl2d.key_is_held(.A)
...
draw :: proc() {
    ...
    if score < len(coins) {
        ...
        text = fmt.tprintf("COMPLETED!!!")
    }
    karl2d.draw_text(text, {500, 10}, 20, karl2d.YELLOW)
}
...
update_actor :: proc(actor: ^Actor) {
    ...
    // Gravity
    target_velocity.y += GRAVITY * karl2d.get_frame_time()
